cc.moduleLoader = {
    ...
    getModuleJsList : function(moduleRoot, modules, cb){
        ...
        cc.loader.load([moduleConfigPath], function(err, results){
            if(err) return cb(err);
            var moduleConfig = results[0];
            var moduleMap = moduleConfig["module"];
            var resultList = [];
            modules = modules || ["default"];//确定需要加载的模块
            for(var i = 0, li = modules.length; i < li; ++i){
                resultList = resultList.concat(this._getJsListOfModule(moduleMap, modules[i], moduleRoot))
            }
            cb(null, resultList);
        });
    },
